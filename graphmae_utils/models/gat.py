    ...
    def __init__(self,
        ...
        if num_layers == 1:
            self.gat_layers.append(GATConv(
                ...
                concat=concat_out,negative_slope=negative_slope, dropout=attn_drop,residual=last_residual, norm=last_norm,aggr=self.aggr))
        else:
            # input projection (no residual)
            self.gat_layers.append(GATConv(
                in_dim, num_hidden, nhead,
                concat=concat_out, negative_slope=negative_slope, dropout=attn_drop, activation=create_activation(activation),residual=residual, norm=norm,aggr=self.aggr))
            # hidden layers
            ...
            self.gat_layers.append(GATConv(
                ...
                concat=concat_out,negative_slope=negative_slope, dropout=attn_drop, activation=last_activation, residual=last_residual, norm=last_norm,aggr=self.aggr))
